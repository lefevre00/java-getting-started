...
public class SearchRoute implements TemplateViewRoute {
	...
	public ModelAndView handle(Request req, Response resp) throws Exception {
    	...
    	String strTomorrow = new SimpleDateFormat("dd/MM/yyyy").format(new Date(dateRecherche.getTime() + (1000 * 60 * 60 * 24)));
    	//if ("POST".equalsIgnoreCase(req.requestMethod())) {
    		if(req.queryParams("nextDay") != null){
    			...
    			strTomorrow = new SimpleDateFormat("dd/MM/yyyy").format(new Date(dateRecherche.getTime() + (1000 * 60 * 60 * 24)));
